...
const DETECTION_ALGORITHMS = {
  ...
  editDistance: (legitimate, candidate, threshold = 3) => {
    ...
    // Special case: legitimate variations (prefixes like "pre-" are different packages)
    const legitimateVariations = [
      { pattern: /^pre/, original: legitimate }, // preact is not a typosquat of react
    ];
    
    const hasLegitimatePrefix = legitimateVariations.some(v => {
      return v.pattern.test(candidate) && !v.pattern.test(legitimate);
    });
    
    return {
      isTyposquat: distance > 0 && distance <= threshold && !hasLegitimatePrefix,
  ...
  typoPatterns: (legitimate, candidate) => {
    const patterns = [
      ...
      // Hyphen/underscore/dot substitution
      { test: () => {
        const normalized1 = candidate.replace(/[-_.]/g, '');
        const normalized2 = legitimate.replace(/[-_.]/g, '');
        return normalized1 === normalized2 && candidate !== legitimate;
      }, desc: "separator-swap" },
  ...
  ensemble: (legitimate, candidate) => {
    // Check for known legitimate variations
    const legitimateVariations = [
      '-es', '-cli', '-core', '-plugin', '-webpack-plugin', '-loader',
      'native', 'timezone', 'js', '.js'
    ];
    
    const isLegitimateVariation = legitimateVariations.some(suffix => {
      return candidate === legitimate + suffix || candidate === `${legitimate}${suffix}`;
    });
    
    // If it's a known legitimate variation, don't flag it
    if (isLegitimateVariation) {
      return {
        isTyposquat: false,
        confidence: 0,
        votes: 0,
